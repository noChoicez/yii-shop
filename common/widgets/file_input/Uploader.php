...
class Uploader
{
	private $multiple;	//是否多文件上传
    private $formName;  //文件表单名
    ...
    */
    ...
    {
    	$file = [];
    	$this->multiple = is_array($_FILES[$this->config['formName']]['name'][$this->config['fieldName']])?true:false;
    	foreach($_FILES[$this->config['formName']] as $k => $v){
    		$file[$k] = ($this->multiple && is_array($v[$this->config['fieldName']]))?getArrayFirstVal($v[$this->config['fieldName']]):$v[$this->config['fieldName']];	
    	}
    	return $file;
    }
    /**
